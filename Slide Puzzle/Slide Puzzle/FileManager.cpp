...
void update_puzzlenumbers (int NxN)
{
	...
	for (unsigned int i = 0; i < vec_to_write.size(); i++)
	{
		...
		writeto_puzzlenumbers << vec_to_write[pos][0] << "x" << vec_to_write[pos][0] << " " << vec_to_write[pos][1] << '\n';
	}
...
vector<vector<int> > read_board_from_txt (int board_size, int puzzle_number)
{
	...
	{
		/* opening input file stream */
		ifstream read_from_txt (filename);
		...
		for (int i = 0; i < board_size; i++)
		{
			...
			for (int j = 0; j < board_size; j++)
			{
				...
				else if (piece_width == 4)
				{
					...
					num_str = char_to_string(num1) + char_to_string(num2) + char_to_string(num3);
				}
...
void update_file_top_ten (int board_size, int puzzle_number, const string & filename, const vector<string> & new_top_ten)
{
	...
	vector<vector<int> > game_board = read_board_from_txt(board_size, puzzle_number);

	/* opening output file stream (deleting all its previous content) */
	ofstream write_to_file (filename);
